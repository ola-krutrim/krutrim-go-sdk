...
func main() {
	...
	params := krutrim.DNSV1ZoneVpcRemoveParams{
		// REQUIRED FIELDS
		Vpcinfo: []string{"Enter VPC Id", "Enter VPC ID 2"},
	}
	...
	// Call API
	err := client.DNS.V1.Zone.Vpc.Remove(ctx, zoneId, params, option.WithResponseInto(&resp))
	if err != nil {
		log.Fatal(err)
	}
	...
	fmt.Printf("VPC removed successfully:\n%s\n", string(responseJSON))
}
